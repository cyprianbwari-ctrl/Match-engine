...
export function mapRosterPlayer(rp) {
  ...
  return {
    ...
    pos: rp.displayPos || rp.pos, club: rp.club || 'Current Club', age: rp.age ?? (20 + (rp.id % 12)), rating,
    ...
    career: [{ season: '2025/26', club: rp.club || 'Current Club', apps: 14 + (rp.id % 4), goals: rp.pos === 'ST' ? 9 : rp.pos.startsWith('AM') ? 5 : 1, assists: rp.pos.startsWith('AM') || rp.pos === 'MC' ? 6 : 2 }],
...
export const todaysSchedule = [
  ...
  { time: '15:00', kind: 'match', title: 'Liverpool vs Newcastle', sub: 'Premier League' },
...
export const squadSnapshot = {
  ...
  injuries: [{ name: 'Nahuel Ibarra', detail: '3 weeks out', rosterId: 4 }],
  suspensions: [{ name: 'Wagner Aguiar', detail: '1 match — accumulated cards', rosterId: 6 }],
  watchlist: [{ name: 'Tyrell Osei', detail: 'Dip in form — monitor minutes', rosterId: 7 }, { name: 'Jamie Colton', detail: 'Heavy fixture load', rosterId: 5 }],
  returning: [{ name: 'Anders Krogh', detail: 'Back in full training', rosterId: 11 }],
};
